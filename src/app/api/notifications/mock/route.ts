	...
	try {
		...
		const mockNotifications = [
			...
			},
			{
				...
				message:
					"Your swimming training session with Coach Sarah is scheduled for tomorrow at 10:00 AM.",
				status: "unread",
				actionUrl: "/training",
				metadata: {
			...
			},
			{
				...
				message:
					"Scheduled maintenance on Dec 20th from 2:00 AM to 4:00 AM. Some features may be unavailable.",
				status: "unread",
				metadata: {
		...
		return NextResponse.json({ error: "Failed to create mock notifications" }, { status: 500 });
	}
}
