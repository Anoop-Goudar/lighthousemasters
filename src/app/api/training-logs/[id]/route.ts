...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		if (!ObjectId.isValid(id)) {
			return NextResponse.json({ error: "Invalid training log ID" }, { status: 400 });
		}
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		const canUpdate = session.user.role === "admin" || existingLog.coachId === session.user.id;
		...
		await db.collection("trainingLogs").updateOne({ _id: new ObjectId(id) }, { $set: updateData });
