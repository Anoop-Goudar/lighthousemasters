              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id={sessionDateId}
                    type="datetime-local"
                    value={formData.sessionDate ? new Date(formData.sessionDate.getTime() - formData.sessionDate.getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ""}
                    onChange={(e) => setFormData({ ...formData, sessionDate: new Date(e.target.value) })}
                    required
                  />
                </div>
          ...
          {trainingLogs.map((log) => (
            <Card key={log._id}>
              <CardHeader>
                ...
                <CardDescription>
                  {new Date(log.sessionDate).toLocaleDateString()} • Duration: {log.performanceMetrics?.duration || 'N/A'} min
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                {log.performanceMetrics && (
                  <div className="flex gap-4 text-sm text-gray-500">
                    {log.performanceMetrics.intensity && <span>Intensity: {log.performanceMetrics.intensity}/10</span>}
                    {log.performanceMetrics.rating && <span>Rating: {log.performanceMetrics.rating}/5 ⭐</span>}
                  </div>
                )}
              </CardContent>
            </Card>
